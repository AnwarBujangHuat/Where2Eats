...
const restaurantCollectionRef = firebase
  .firestore()
  .collection(ConstString.RESTAURANT);
...
export const PopulateRestaurantList = createAsyncThunk(
  ...
  async (request, {dispatch, rejectWithValue}) => {
    ...
    if (!onSuccess) {
      return rejectWithValue({result: onSuccess, data: data});
    }
...
const requestAddNewFoodItem = (id, foodItem) =>
  new Promise((resolve, reject) => {
    restaurantCollectionRef
      .doc(id)
      .update('food', arrayUnion(foodItem))
      .then(
        () => resolve({onSuccess: true}),
        () => resolve({onSuccess: false}),
      );
...
const requestRemoveFoodItem = (id, foodItem) =>
  new Promise((resolve, reject) => {
    restaurantCollectionRef
      .doc(id)
      .update('food', arrayRemove(foodItem))
      .then(
        () => resolve({onSuccess: true}),
        () => resolve({onSuccess: false}),
      );
...
export const updateUserFCM = createAsyncThunk(
  'updateUser',
  async (request, {dispatch, rejectWithValue}) => {
    const {userToken: token, deviceId, userId: uid} = request;
    const {onSuccess: successUpdateFCM} = await requestUpdateFCM(
      uid,
      token,
      deviceId,
    );
...
const requestUpdateFCM = (uid, token, deviceId) =>
  new Promise((resolve, reject) => {
    fcmCollectionRef
      .doc(deviceId)
      .set(
...
export const verifyUserToken = createAsyncThunk(
  'verifyToken',
  async (request, {dispatch, rejectWithValue}) => {
    const {token, deviceId} = request;
    const {onSuccess: userExistInDatabase, data} = await requestVerifyToken(
      token,
      deviceId,
    );
    if (!userExistInDatabase) {
      return rejectWithValue({
        result: userExistInDatabase,
...
const requestVerifyToken = (token, deviceId) =>
  new Promise((resolve, reject) => {
    fcmCollectionRef
      ...
      .then(querySnapshot => {
        ...
        if (querySnapshot.empty) {
          return resolve({onSuccess: false, data: 'No data'});
        }
